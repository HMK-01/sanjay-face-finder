...
if st.button("Start Search Engine", type="primary", use_container_width=True):
    if not uploaded_face or not uploaded_video:
        ...
        while video_capture.isOpened():
            ...
            if len(match_records_list) >= MAX_MATCHES:
                st.warning(f"Memory Guard Active: Displaying the first {MAX_MATCHES} strong matches to prevent server crashes.")
                break
        ...
        if len(match_records_list) == 0:
            st.info("No matching face instances found. Try sliding down the 'Match Accuracy' setting if the person is moving fast or poorly lit.")
        else:
